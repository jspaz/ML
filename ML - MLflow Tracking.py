...
with mlflow.start_run(run_name="LR-Single-Feature") as run:
    ...
    # Log model
    mlflow.spark.log_model(pipeline_model, "model", input_example=train_df.limit(5).toPandas(), signature = signature) 
...
input_schema = Schema(
    ...
    ColSpec("string", "cancellation_policy"),
    ColSpec("string", "instant_bookable"),
    ColSpec("double", "host_total_listings_count"),
    ...
    ColSpec("double", "accommodates"),
    ColSpec("double", "bathrooms"),
    ColSpec("double", "bedrooms"),
    ColSpec("double", "beds"),
    ColSpec("string", "bed_type"),
    ColSpec("double", "minimum_nights"),
    ColSpec("double", "number_of_reviews"),
    ColSpec("double", "review_scores_rating"),
    ColSpec("double", "review_scores_accuracy"),
    ColSpec("double", "review_scores_cleanliness"),
    ColSpec("double", "review_scores_checkin"),
    ColSpec("double", "review_scores_communication"),
    ColSpec("double", "review_scores_location"),
    ColSpec("double", "review_scores_value"),
    ColSpec("double", "price"),
    ColSpec("double", "bedrooms_na"),
    ColSpec("double", "bathrooms_na"),
    ColSpec("double", "beds_na"),
    ColSpec("double", "review_scores_rating_na"),
    ColSpec("double", "review_scores_accuracy_na"),
    ColSpec("double", "review_scores_cleanliness_na"),
    ColSpec("double", "review_scores_checkin_na"),
    ColSpec("double", "review_scores_communication_na"),
    ColSpec("double", "review_scores_location_na"),
    ColSpec("double", "review_scores_value_na")
    ]
)
output_schema = Schema([ColSpec("long")])
signature = ModelSignature(inputs=input_schema, outputs=output_schema)
...
with mlflow.start_run(run_name="LR-All-Features") as run:
    ...
    train_df.toPandas().hist(column="price", bins=100)
    fig = plt.gcf()
    mlflow.log_figure(fig, f"demos.datos_price_normal.png")
    plt.show()
 #   dbfs:/demos.datos
...
with mlflow.start_run(run_name="LR-Log-Price") as run:
    ...
    fig = plt.gcf()
    mlflow.log_figure(fig, f"demos_datos_log_normal.png")
    plt.show()
...
# MAGIC
# MAGIC
# MAGIC ### Saving model in Unity Catalog
# MAGIC
# MAGIC
